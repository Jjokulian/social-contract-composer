...
- A contract says what it is in its **case**: `proposed`, one to compose with now (the default); `historical`, a record of what was, pinned to a period and to its sources; or `fictive`, an example that shows the structure and was never signed by anyone. A demesne carries the case of the milli revision it pins, so a historical demesne stays historical however its milli is revised later. The views that compose show the proposed contracts unless their address asks for more (`?case=historical`, `?case=all`), so a record of what was never drifts into a milli meant to be signed; the Globe and the Timeline show every case, which is what they are for.
